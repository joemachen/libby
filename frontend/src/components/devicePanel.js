/**
 * devicePanel — device status indicator in the header.
 *
 * Polls GET /api/device/status every POLL_MS milliseconds.
 * On connect/disconnect it toggles the CSS class "device-connected" on #app,
...
import { getDeviceStatus, sendToDevice, bulkSendToDevice, ejectDevice } from "../api.js";
...
export const DevicePanel = {
     ...
     * @param {{ onSent?: Function, onError?: Function, onConnected?: Function, onDisconnected?: Function, onShelfOpen?: Function, onEjected?: Function, onBulkSent?: Function }} callbacks
     */
    ...
    /**
     * Send a single book to the device. Called by app.js from the grid's onSend handler.
     * @param {string} bookId
     * @param {string} bookTitle
     */
    async sendBook(bookId, bookTitle) {
        if (!this._device) {
            this._callbacks.onError?.("No device connected.");
            return;
        }
        try {
            const result = await sendToDevice(bookId);
    ...
    /**
     * Send multiple books to the device. Called by app.js from the grid's onBulkSend handler.
    ...
    async sendBooks(bookIds) {
        ...
        try {
            const data = await bulkSendToDevice(bookIds);
    ...
    async _poll() {
        try {
            ...
            if (!wasConnected && nowConnected) {
                document.getElementById("app")?.classList.add("device-connected");
                this._callbacks.onConnected?.(this._device);
            } else if (wasConnected && !nowConnected) {
                document.getElementById("app")?.classList.remove("device-connected");
    ...
    _render(device) {
        if (!device) {
            this.el.innerHTML =
                `<span class="kobo-pill kobo-pill-off">No device connected</span>`;
            return;
        }
...
<button class="btn-icon kobo-action-btn" data-kobo-action="shelf" title="View device shelf">
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>
  </svg>
</button>
<button class="btn-icon kobo-action-btn kobo-eject-btn" data-kobo-action="eject" title="Safely eject device">
