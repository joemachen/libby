/**
 * koboShelf — a right-side drawer that lists all EPUBs on the connected Kobo,
 * cross-referenced with the local library.
 *
 * Usage:
 *   KoboShelf.init()          — call once at startup (attaches overlay click handler)
 *   KoboShelf.open()          — fetch books and slide the drawer in
 *   KoboShelf.close()         — slide the drawer out
 */

import { getKoboBooks } from "../api.js";

export const KoboShelf = {
    _drawer: null,
    _overlay: null,
    ...
    init() {
        this._drawer  = document.getElementById("kobo-shelf");
        this._overlay = document.getElementById("kobo-shelf-overlay");
    ...
    /** Fetch Kobo books and open the drawer. */
    async open() {
        ...
        try {
            this._books = await getKoboBooks();
            this._renderBooks(this._books);
        } catch (err) {
...
<div class="shelf-header">
  <h2 class="shelf-title">Kobo Shelf</h2>
...
<div class="shelf-error">${_esc(msg)}</div>`;
    ...
    _renderBooks(books) {
        ...
        this._drawer.innerHTML = `
<div class="shelf-header">
  <h2 class="shelf-title">Kobo Shelf <span class="shelf-count">${books.length} book${books.length !== 1 ? "s" : ""}</span></h2>
  <button class="btn-icon shelf-close-btn" aria-label="Close shelf">✕</button>
</div>
